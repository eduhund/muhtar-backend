...
export default async function archiveTime(id: string, currentUser: User) {
  ...
  await canArchiveTime(currentMembership, existingTime, project);

  await existingTime.archive(currentMembership);
